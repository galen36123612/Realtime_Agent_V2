...
// 0522 More apple-style microphone icon
...
export interface TranscriptProps {
  ...
  onSendMessage: () => void;
  canSend: boolean;
  handleTalkButtonDown?: () => void;  // Add these props from BottomToolbar
  handleTalkButtonUp?: () => void;
  isPTTActive?: boolean;
  isPTTUserSpeaking?: boolean;
}
  ...
  return (
    <div className="flex flex-col flex-1 bg-white min-h-0 rounded-xl">
      ...
      <div className="p-4 flex items-center gap-x-2 flex-shrink-0 border-t border-gray-200">
        ...
        {handleTalkButtonDown && handleTalkButtonUp && (
          <button
            onMouseDown={handleTalkButtonDown}
            onMouseUp={handleTalkButtonUp}
            onTouchStart={handleTalkButtonDown}
            ...
            } ${!isPTTActive ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "cursor-pointer hover:bg-gray-300"}`}
          ...
          >
            <svg
              ...
              viewBox="0 0 24 24"
              fill="none"
              stroke={isPTTActive ? (isPTTUserSpeaking ? "currentColor" : "currentColor") : "#9CA3AF"}
              strokeWidth="2"
              strokeLinecap="round"
        ...
        <button
          onClick={onSendMessage}
          disabled={!canSend || !userText.trim()}
          className="bg-gray-900 text-white rounded-full p-2 disabled:opacity-50"
        >
          <Image src="arrow.svg" alt="Send" width={24} height={24} />
        </button>
      </div>
